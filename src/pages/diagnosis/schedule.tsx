...
const REGION_GROUPS: { label: string; options: string[] }[] = [
  { label: "서울", options: [
    "강남구", "강동구", "강북구", "강서구(서울)", "관악구", "광진구", "구로구", "금천구",
    "노원구", "도봉구", "동대문구", "동작구", "마포구", "서대문구", "서초구", "성동구",
    "성북구", "송파구", "양천구", "영등포구", "용산구", "은평구", "종로구", "중구(서울)", "중랑구",
  ]},
  { label: "부산", options: [
    "강서구(부산)", "금정구", "남구(부산)", "동구(부산)", "동래구", "부산진구", "북구(부산)",
    "사상구", "사하구", "서구(부산)", "수영구", "연제구", "영도구", "중구(부산)", "해운대구", "기장군",
  ]},
  { label: "대구", options: [
    "남구(대구)", "달서구", "동구(대구)", "북구(대구)", "서구(대구)", "수성구", "중구(대구)", "달성군",
  ]},
  { label: "인천", options: [
    // 인천 서구는 다른 시/도의 동명 "서구"와 겹쳐서 "서해구"로 개명됨(카카오 지오코딩도 이미
    // "서해구"로 반환) — 신규 선택은 이 이름으로만 하고, 기존에 "서구(인천)"로 저장된
    // 진단사 데이터는 담당지역에 "서해구"를 추가로 등록해줘야 자동배정이 계속 잡힘.
    "계양구", "남동구", "동구(인천)", "미추홀구", "부평구", "서해구", "연수구", "중구(인천)", "강화군", "옹진군",
  ]},
  { label: "광주", options: [
    "광산구", "남구(광주)", "동구(광주)", "북구(광주)", "서구(광주)",
  ]},
  { label: "대전", options: [
    "대덕구", "동구(대전)", "서구(대전)", "유성구", "중구(대전)",
  ]},
  { label: "울산", options: [
    "남구(울산)", "동구(울산)", "북구(울산)", "중구(울산)", "울주군",
  ]},
  { label: "세종", options: ["세종"] },
  { label: "경기", options: [
          ...
          <div className="flex-1 space-y-5">
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm font-bold text-gray-700">가능 지역</p>
                <p className="text-xs text-gray-400">{regions.length}개 선택</p>
              </div>
              <div className="space-y-3 max-h-[420px] overflow-y-auto pr-1">
                {REGION_GROUPS.map(group => {
                  const selectedCount = group.options.filter(r => regions.includes(r)).length;
                  const allOn = selectedCount === group.options.length;
                  return (
                    <div key={group.label} className="border border-gray-100 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        <button
                          onClick={() => setRegions(prev =>
                            allOn
                              ? prev.filter(x => !group.options.includes(x))
                              : [...prev.filter(x => !group.options.includes(x)), ...group.options]
                          )}
                          className="text-xs font-bold text-gray-500 hover:text-violet-600 transition-colors"
                        >
                          {group.label}
                        </button>
                        {selectedCount > 0 && (
                          <span className="text-[10px] font-semibold text-violet-500">{selectedCount}/{group.options.length}</span>
                        )}
                      </div>
